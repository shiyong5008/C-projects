...
queue_t* create_queue(unsigned int _capacity) {
	...
	myQueue->back = 0;
	myQueue->front = 0; //assign tail and front pointor to same slot
	myQueue->size = 0;
	myQueue->capacity = _capacity; // How big our queue can be.
...
int queue_empty(queue_t* q) {
	if (t.size == 0) // & front == back //check the queue is empty.
	{
		return 1;
	}
	else
...
int queue_full(queue_t* q) {
	if (t.size >= capacity) //(back+1)%size == front //check the queue is full.
	{
		return 1;
	}
	else
...
int queue_enqueue(queue_t* q, int item) {
	...
	{
		*back = &item; back = (back + 1) % size;
	}//insert data to the queue end and back pointer to next slot
	return 0;
}
...
int queue_dequeue(queue_t* q) {
	...
	{
		front = (front + 1) % size;
	} //put front pointer to the next slot after dequeue the item.
...
unsigned int queue_size(queue_t* q) {
	...
	size_t queue_sizepqueue_t* q);
	if currentsize = (back - front + size) % size;//count current queue size
	{
		return current size; 
	}
	else
	{
		exit(1);
	}  //if can not return current size, call exit.
}
// Free queue
