...
*edit me and add in a short description of the command and its usage* cd for directory change; help for print out command; exit for terminate.

DESCRIPTION:

*Write a longer description of what he command does*
int lshell_cd cd this is the command line for change directories.
int lshell_help help this is the print out for the command line.
int lshell_exit exit when the shell need terminate.

NOTES:

*Enter two or three sentences (minimum) in how you implemented the built-in command.* build in with mumbers set and make the shell running non stop untill exit.
