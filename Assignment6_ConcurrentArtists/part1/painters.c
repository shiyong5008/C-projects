...
void* paint(void* args){
	...
	for(int i =0; i < 5000; ++i){
        ...
        int rc = pthread_mutex_trylock(&canvas[painter->x][painter->y].lock);
	    if(rc != 0)
        {
            //not able to get lock retry
            //painter->x = currentX;
            //painter->y = currentY;
            continue;
        }
...
int main(){
    ...
    artist_t* moreArtists = (artist_t*)malloc(50 * sizeof(artist_t));
    for(int i =0; i < rookieArtists; ++i){
        moreArtists[i].x = rand() % CANVAS_HEIGHT;
        moreArtists[i].y = rand() % CANVAS_HEIGHT;
        moreArtists[i].r = rand() % CANVAS_HEIGHT;
        moreArtists[i].g = rand() % CANVAS_HEIGHT;
        moreArtists[i].b = rand() % CANVAS_HEIGHT;
