...
int main(){
	...
	myargv2[0]="/bin/clear";	// Our first argument is the program we want to launch.
	myargv2[1]="-F";		// Any additional arguments (i.e. flags) we want to make use of.
	myargv2[2]=NULL; 	// Terminate the argument list--similar to how we would terminate a character string.
				// (i.e. Set the last argument to NULL if we have no more flags.)
	if(fork()==0){
		// Executes command from child then terminates our process
		// Note: There are other 'exec' functions that may be helpful.
        ...
        myargv3[0]="/bin/make"; // Our first argument is the program we want to launch.
        myargv3[1]="-F";         // Any additional arguments (i.e. flags) we want to make use of.
        myargv3[2]=NULL;         // Terminate the argument list--similar to how we would terminate a character string.
                                // (i.e. Set the last argument to NULL if we have no more flags.)
        if(fork()==0){
                // Executes command from child then terminates our process
                // Note: There are other 'exec' functions that may be helpful.
